...
export default class ReactForm extends React.Component {
      ...
      if (item.element === 'Checkboxes' || item.element === 'RadioButtons') {
        item.options.forEach((option) => {
          let $option = ReactDOM.findDOMNode(
            this.refs[item.field_name].refs[`child_ref_${option.key}`]
          )
          if (
  ...
  _getSignatureImg(item) {
    let $canvas_sig = this.refs[item.field_name].refs[
  ...
  render() {
    ...
    let items = data_items.map((item) => {
      switch (item.element) {
        ...
        case 'Paragraph':
          return (
            <Paragraph mutable={true} key={`form_${item.id}`} data={item} />
          )
        case 'Label':
          return <Label mutable={true} key={`form_${item.id}`} data={item} />
        case 'LineBreak':
          return (
            <LineBreak mutable={true} key={`form_${item.id}`} data={item} />
          )
        case 'TextInput':
          return (
            <TextInput
        ...
        case 'HyperLink':
          return (
            <HyperLink mutable={true} key={`form_${item.id}`} data={item} />
          )
        case 'Download':
          return (
            <Download
