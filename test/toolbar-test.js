...
var Toolbar = require('../src/toolbar').default
...
describe('<Toolbar />', function () {
  ...
  before('render and locate element', function () {
    this.toolBar = TestUtils.renderIntoDocument(<Toolbar />)
  ...
  it('should display default options when none are passed in', function () {
    assert.equal(
      this.inputElement.querySelector('ul').children.length,
      this.toolBar.state.items.length
    )
  })
  ...
  it('should display certain toolbar items that are passed in', function () {
    ...
    this.toolBar = TestUtils.renderIntoDocument(<Toolbar items={items} />)
